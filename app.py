...
from llm import parse_seller, parse_buyer, match_sellers, format_quote_request, format_quote_response, analyze_photo_for_service
import json
import base64
import logging
...
def search_photo():
    ...
    photo_ext = ext if ext in ("jpg", "jpeg", "png", "gif", "webp") else "jpg"
    photo_filename = f"search_{user.id}.{photo_ext}"
    ...
    log.info(f"PHOTO_SEARCH  user={user.id}  service={analysis.get('service')}  desc={analysis.get('description')}")
    ...
    parsed = {
        ...
        "unrecognized_cities": [],
        "_raw_query": "",
        "_photo_description": analysis.get("description", ""),
